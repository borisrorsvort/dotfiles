...
2. **Detect brownfield vs greenfield**:
   - Run `explore` agent (auto/best-fast): check if cwd has existing source code, package files, or git history
...
**Scoring prompt** (use auto/best-reasoning model, temperature 0.1 for consistency):
...
1. **Generate the specification** using auto/best-reasoning model with the prompt-safe transcript. If the full interview transcript or initial context is too large, include the summary plus all concrete decisions, acceptance criteria, unresolved gaps, and ontology snapshots; never overflow the prompt with raw oversized context.
...
- Use `Task(subagent_type="oh-my-claudecode:explore", model="auto/best-fast")` for brownfield codebase exploration (run BEFORE asking user about codebase)
- Use auto/best-reasoning model (temperature 0.1) for ambiguity scoring — consistency is critical
...
```json
{
  "omc": {
    "deepInterview": {
      ...
      "scoringModel": "auto/best-reasoning"
    }
  }
}
```
