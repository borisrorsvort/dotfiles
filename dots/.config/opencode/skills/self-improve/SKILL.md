...
|------|------|-----------|-------|
| Research | Codebase analysis + hypothesis generation | general-purpose Agent | auto/best-reasoning |
| Planning | Hypothesis → structured plan | oh-my-claudecode:planner | auto/best-reasoning |
| Architecture Review | 6-point plan review | oh-my-claudecode:architect | auto/best-reasoning |
| Critic Review | Harness rule enforcement | oh-my-claudecode:critic | auto/best-reasoning |
| Execution | Implement plan + run benchmark | oh-my-claudecode:executor | auto/best-reasoning |
| Git Operations | Atomic merge/tag/PR | oh-my-claudecode:git-master | auto/best-coding |
| Goal Setup | Interactive interview | (directly in this skill) | N/A |
| Benchmark Setup | Create + validate benchmark | custom agent | auto/best-reasoning |
...
6. If benchmark not set → read `si-benchmark-builder.md` from this skill directory, spawn a custom Agent(model=auto/best-reasoning) with its content as prompt. The agent surveys the repo, creates or wraps a benchmark, validates 3x, and records baseline.
...
Spawn 1 general-purpose Agent(model=auto/best-reasoning) with the content of `si-researcher.md` as prompt.
...
Spawn N `oh-my-claudecode:planner`(model=auto/best-reasoning) agents in parallel (N = `number_of_agents` from settings).
...
For each approved plan, spawn `oh-my-claudecode:executor`(model=auto/best-reasoning) in parallel.
