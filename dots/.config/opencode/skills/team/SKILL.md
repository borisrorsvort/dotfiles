...
| --------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **team-plan**   | `explore` (auto/best-fast), `planner` (auto/best-reasoning) | `analyst` (auto/best-reasoning), `architect` (auto/best-reasoning)                                                                    | Use `analyst` for unclear requirements. Use `architect` for systems with complex boundaries.                                                                                                      |
| **team-prd**    | `analyst` (auto/best-reasoning)                    | `critic` (auto/best-reasoning)                                                                                         | Use `critic` to challenge scope.                                                                                                                                                                  |
| **team-exec**   | `executor` (auto/best-coding)                 | `executor` (auto/best-reasoning), `debugger` (auto/best-coding), `designer` (auto/best-coding), `writer` (auto/best-fast), `test-engineer` (auto/best-coding) | Match agent to subtask type. Use `executor` (model=auto/best-reasoning) for complex autonomous work, `designer` for UI, `debugger` for compilation issues, `writer` for docs, `test-engineer` for test creation. |
| **team-verify** | `verifier` (auto/best-coding)                 | `test-engineer` (auto/best-coding), `security-reviewer` (auto/best-coding), `code-reviewer` (auto/best-reasoning)                          | Always run `verifier`. Add `security-reviewer` for auth/crypto changes. Add `code-reviewer` for >20 files or architectural changes. `code-reviewer` also covers style/formatting checks.          |
| **team-fix**    | `executor` (auto/best-coding)                 | `debugger` (auto/best-coding), `executor` (auto/best-reasoning)                                                                  | Use `debugger` for type/build errors and regression isolation. Use `executor` (model=auto/best-reasoning) for complex multi-file fixes.                                                                          |
...
3. **Cost mode affects model tier.** In downgrade: `auto/best-reasoning` agents to `auto/best-coding`, `auto/best-coding` to `auto/best-fast` where quality permits. `team-verify` always uses at least `auto/best-coding`.
4. **Risk level escalates review.** Security-sensitive or >20 file changes must include `security-reviewer` + `code-reviewer` (auto/best-reasoning) in `team-verify`.
...
| `planner`       | claude          | `HIGH` (auto/best-reasoning)             |
| `analyst`       | claude          | `HIGH` (auto/best-reasoning)             |
| `executor`      | claude          | `MEDIUM` (auto/best-coding)         |
...
An empty `team.roleRouting` preserves pre-patch behavior: every worker is Claude, model tiers follow `routing.tierModels`, and `/team 3:executor ...` still spawns three Claude auto/best-coding executors.
