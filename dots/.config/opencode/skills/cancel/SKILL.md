...
description: Cancel any active OMC mode (autopilot, ralph, team, ultragoal, plan)
...
- **Team**: Requests shutdown from all teammates through the active team/conversation surface, waits for responses/timeouts, clears OMC team state, clears linked ralph if present.
- **Ultragoal**: Stops the durable multi-goal workflow
- **Plan/Ralph linked**: Cancels the active plan or ralph loop, clears linked state
...
it only removes state files to unblock the session. Linked modes (e.g. autopilot→ralph)
must be cleared separately by running the fallback once per mode.

Replace `MODE` with the specific mode (e.g. `ralplan`, `ralph`, `team`).
